// VetJobs verification engine (shared core).
...
export function verifyJob(input: VerifyInput): VerifyResult {
  const { text = '', email = '', company = '', companyKnown = null, sourceTrust = 0 } = input;
  let score = 50;
  ...
  if (!noFee && /fee|pay (?:₦|n|ngn)?\s?\d|deposit/i.test(t)) {
    score -= 35; reasons.push('Asks for money / a fee before employment — the #1 scam sign.');
  }
